...
{
    public partial class EmpChangePassword : UserControl
    {
        ...
        private void ChangePassButton_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrWhiteSpace(CurrentPassTextBox.Text) && !String.IsNullOrWhiteSpace(NewPassTextBox.Text) && Regex.IsMatch(CurrentPassTextBox.Text, passwordpattern) == true && Regex.IsMatch(NewPassTextBox.Text, passwordpattern) == true)
            {
                try
                {
                    ...
                    sqlquery = "select emp_password from cinema.Employee where emp_password = '" + CurrentPassTextBox.Text + "' and emp_id = " + custid + "";
                    ds = f.GetData(sqlquery);
                    if (ds.Tables[0].Rows.Count != 0)
                    {
                        sqlquery = "update cinema.Employee set emp_password = '" + NewPassTextBox.Text + "' where emp_id =" + custid + "";
                        f.SetData(sqlquery);
        ...
        private void EmpChangePassword_Enter(object sender, EventArgs e)
        {
            CurrentPassTextBox.Clear();
            NewPassTextBox.Clear();
        }

        private void EmpChangePassword_Load(object sender, EventArgs e)
        {
            CurrentPassTextBox.Clear();
            NewPassTextBox.Clear();
        }
